...
func TestLustreParams_FileSystemNameValidation(t *testing.T) {
	...
	p["fileSystemName"] = "fs_1"
	parsed, err := parseParams(t, p, &MockOCIIdentityClient{ads: []string{"phx:PHX-AD-2"}})
	if err != nil {
		t.Fatalf("unexpected err: %v", err)
	}
	if parsed.FileSystemName != "fs_1" {
		t.Fatalf("expected fs_1, got %s", parsed.FileSystemName)
	}
}
...
func TestDeriveDefaultFSName_Empty(t *testing.T) {
	...
	if got != "lustrefs" {
		t.Fatalf("expected 'lustrefs' for empty volumeName, got %q", got)
	}
}
